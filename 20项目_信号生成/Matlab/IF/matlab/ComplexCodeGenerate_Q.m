function [Hn,PhaseHnPulse] = ComplexCodeGenerate_Q(PulseNumTh,Npw,m_SubStepNum,m_PCMCodeIndex,m_fs,m_CodeWidth,m_SubPulsePCMCode,m_CodeNumAll)
    ...
    for kk = PulseNumTh * Npw *m_SubStepNum:(PulseNumTh * Npw * m_SubStepNum + (2^m_PCMCodeIndex - 1) * m_SubStepNum-1)%%
	  ...
	  Hn = cos(PhaseHnPulse*2*pi/2^16) + sqrt(-1)*sin(PhaseHnPulse*2*pi/2^16) ;
